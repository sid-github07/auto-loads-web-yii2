...
/**
 * This controller is responsible for setting car_transporter.days_adv car_transporter.submit_time_adv and car_transporter.car_pos_adv to NULL / zero values
 ...
 *
 * Class TransporterCronController
 * @package console\controllers
 */
class TransporterCronController extends Controller
{
    use ConsoleMethods;
    ...
    {
        Yii::info('TransporterCronController is initiated');
        $start = new \DateTime();
        $errorMessage = null;
        ...
        try {
            ...
            $sql = 'UPDATE '. CarTransporter::tableName() .' SET days_adv=:days, car_pos_adv=:pos, submit_time_adv=:time WHERE submit_time_adv is NOT NULL AND days_adv != 0 AND DATE_FORMAT(DATE_ADD(submit_time_adv, INTERVAL days_adv DAY), \'%Y-%m-%d %H:%i:%s\') <= :now LIMIT 1000;';
