...
<section class="<?php echo $sectionClass; ?>">
    <?php if (empty($directLoads) || empty($loads)): ?>
    ...
    <?php else: ?>
        <?php foreach ($directLoads as $directLoad): ?>
            <?php if (is_array($directLoad)): ?>
                <?php foreach ($directLoad as $id => $searchRequest): ?>
                    <?php if (ControllerTrait::isActiveMenuItem('load', 'suggestions') && is_array($searchRequest)): ?>
                        <section class="search-results-container">
                            ...
                            <span class="search-results-content clearfix">
                                <span class="load-city">
                                    <?php echo LoadCity::getFormattedSuggestionCity($searchRequest['loadCityRequest']); ?>
                                </span>
                                ...
                                <span class="unload-city">
                                    <?php echo LoadCity::getFormattedSuggestionCity($searchRequest['unloadCityRequest']); ?>
                                </span>

                                <span class="load-quantity">
                                    <?php echo ' (' . $searchRequest['loadRequestQuantity'] . ' ' . Yii::t('element', 'IA-C-81') . ')'; ?>
                                </span>
                            </span>
                        </section>
                    <?php endif; ?>
                                ...
                                <?php foreach ($loads as $load): ?>
                                    <?php if ($load->id == $id): ?>
                                        <tr class="content-row">
                                            <td class="text-center" data-title="<?php echo Yii::t('element', 'IA-C-23'); ?>">
                                                <?php
