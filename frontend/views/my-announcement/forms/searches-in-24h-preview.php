...
<?php if (count($searchesLast24h)) : ?>
    <div class="table-responsive custom-table">
        <table class="table" id="<?php echo $formName; ?>">
            <tbody>
            <?php foreach ($searchesLast24h as $entity):
                $viewed = isset($opened[$entity['user_id']]);
                $diff = (new DateTime())->setTimestamp($entity['updated_at'])->diff((new DateTime()));
                ?>
                <tr <?php if ($viewed) {
                    ...
                    <td>
                        ...
                        foreach ($loadCities as $city) {
                            foreach ($entity['source']['load'] as $ctc) {
                        ...
                        $cityFrom = City::findOne($from['city_id']); // load city in similar load
                    ...
                    <td class="load-contacts">
                        ...
                        foreach ($unloadCities as $city) {
                            foreach ($entity['source']['unload'] as $ctc) {
                                $newDistance = Cities::getDistance($city['city_id'], $ctc['city_id']);
                                if (is_null($distance) || $newDistance < $distance) {
                                    $distance = $newDistance;
                                    $closest = $ctc;
                        ...
                        $city = City::findOne($closest['city_id']);
                        $cityTo = City::findOne($to['city_id']); //unload city in similar load
                        if (intval($distance) > 0) {
                            if ($cityTo->isCountry()) {
                                $howFarAway = Yii::t('element', 'is located in {0}', [
                                    $cityTo->name
...
<?php else : ?>
    <?php echo Html::tag('span', Yii::t('element', 'no_24h_searches')); ?>
<?php endif; ?>
